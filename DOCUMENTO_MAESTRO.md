...
`neto_aprobado_mp` y `neto_financiero_total_mp` no son equivalentes. El primero representa pagos aprobados comparables. Para el Bloque B, el segundo se calcula como pagos aprobados más devoluciones, reclamos, disputas y otros impactos independientes que efectivamente modifican el neto comparable. `PAGO_ENVIO` se conserva como desglose trazable ya incluido en el pago aprobado y no se suma nuevamente. `PAYOUT` no integra `neto_financiero_total_mp` ni `impacto_otros_mp` y permanece separado como movimiento de fondos aun si excepcionalmente informa un ID de orden. Por eso la ausencia de `neto_aprobado_mp` no implica automáticamente datos críticos incompletos.
...
## Actualización: organización visual final para presentación

La interfaz de resultados del control consolidado de tres fuentes queda organizada en `st.tabs(["Resumen ejecutivo", "Control por operación", "Auditoría y descargas"])`. Esta organización es exclusivamente visual y de experiencia de usuario: no modifica normalización, contratos, estados, universos, prioridades, agrupamiento, fórmulas ni importes `Decimal` del dominio.

**Resumen ejecutivo** contiene cobertura temporal legible en formato tabular, conclusión ejecutiva breve, KPIs de los bloques A, B, C y D y resumen compacto de revisiones. Las explicaciones de alcance permanecen disponibles en expanders cerrados para evitar una pantalla inicial técnica.

**Control por operación** contiene vista, filtros, tabla operativa, selector de operación, información del control, explicación de cómo se obtuvo el resultado y trazabilidad técnica cerrada por defecto. La tabla conserva prioridad visual para grupo/orden, estado, fuentes disponibles, venta ML oficial, neto esperado ML, costo de productos, neto aprobado MP, diferencia ML–MP, utilidad preliminar y revisión.

**Auditoría y descargas** contiene cobertura monetaria entre fuentes, utilidad preliminar auditable, residual oficial de Mercado Libre, puente de importes, distribución temporal MP, revisiones consolidadas, descargas consolidadas y la auditoría histórica Eccomapp–Mercado Pago en un expander cerrado. Las descargas del control consolidado se diferencian explícitamente con prefijos `kiki_control_consolidado_3_fuentes_`, `kiki_control_excepciones_consolidadas_` y `kiki_control_revisiones_consolidadas_`; las históricas usan `kiki_control_historico_eccomapp_mp_` y deben indicar que no son el control consolidado actual de tres fuentes.

## Actualización: cierre final de consistencia funcional y exportación consolidada

El control consolidado de tres fuentes incorpora una clasificación explícita para ventas oficiales de Mercado Libre con fuente presente pero dato monetario requerido ausente. Si existe venta oficial y falta `Total (ARS)`, el resultado se clasifica como `TOTAL_ML_AUSENTE`, visible para cliente como **Venta oficial sin Total (ARS)**. Este estado es primario, auditable y mutuamente excluyente; se resuelve con prioridad determinista después de `SIN_VENTA_OFICIAL` y antes de `SIN_COSTO_PRODUCTO`. El resultado requiere revisión, no cuenta como completo monetario, no calcula `diferencia_ml_mp`, no calcula `utilidad_preliminar_control` y conserva los importes informados por Eccomapp y Mercado Pago cuando existan. La explicación debe indicar prudentemente que existe venta oficial, pero falta el importe `Total (ARS)` necesario para el control.

Las fórmulas financieras validadas no cambian y deben seguir usando `Decimal` y datos presentes: `diferencia_ml_mp = neto_aprobado_mp - total_informado_ml` y `utilidad_preliminar_control = total_informado_ml - costo_productos_eccomapp`. `TOTAL_ML_AUSENTE` no se clasifica como diferencia contable y se incorpora a las revisiones consolidadas como categoría independiente de **Fuente faltante** y **Revisión financiera**.

El resumen ejecutivo y la auditoría distinguen costo total informado por Eccomapp, costo Eccomapp utilizado para utilidad preliminar y costo Eccomapp excluido del universo calculable. Las identidades obligatorias son `costo_excluido = costo_total_eccomapp - costo_eccomapp_universo_calculable` y `utilidad_preliminar = neto_ml_universo_calculable - costo_eccomapp_universo_calculable`. La interfaz debe mostrar grupos incluidos y excluidos, motivos de exclusión y ayudas que indiquen origen Eccomapp, columna `Costo Total (Con IVA) ($)`, universo, fórmula, motivo de exclusión y limitación operativa. La utilidad continúa siendo preliminar de control y no ganancia contable, fiscal ni impositiva definitiva.

La exportación Excel consolidada incluye en **Control por operación** las columnas **Costo de productos Eccomapp** y **Utilidad preliminar**. El costo proviene de Eccomapp `Costo Total (Con IVA) ($)`, se prepara con `Decimal`, se escribe como número con formato monetario argentino y queda vacío cuando está ausente. Los identificadores permanecen como texto, se mantiene orden determinista, prevención de fórmulas ejecutables y exclusión de PII, contenido crudo y hashes completos. El resumen/auditoría y el diccionario de cálculos incorporan costo total Eccomapp, costo usado, costo excluido, grupos calculables, grupos excluidos, fórmula exacta de utilidad preliminar y motivos de exclusión.


La contabilización del estado `TOTAL_ML_AUSENTE` se expone mediante el contador explícito `total_total_ml_ausente`. Este contador forma parte de la partición primaria junto con todos los demás estados del dominio, debe cerrar exactamente contra `total_resultados` y se muestra con la etiqueta cliente **Venta oficial sin Total (ARS)** en conclusión/alcance ejecutivo, Bloque D de calidad y pendientes y resumen del Excel consolidado. La ayuda de cobertura de utilidad debe aclarar que no alcanza con tener fuentes presentes: se requieren `Total (ARS)` válido de Mercado Libre oficial y `Costo Total (Con IVA) ($)` de Eccomapp.
