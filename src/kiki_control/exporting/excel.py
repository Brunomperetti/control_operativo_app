...
def _escribir_resumen_consolidado(ws: Worksheet, reporte: ReporteControlConsolidado, tipo: str) -> None:
    ws.append(["Campo", "Valor"])
    diag = diagnosticar_control_consolidado(reporte)
    for fila in (("Nombre", "Kiki Control Financiero"), ("Tipo de reporte", tipo), ("Versión de regla", reporte.version_regla), ("Tolerancia", _decimal_o_vacio(reporte.tolerancia)), ("Total grupos", reporte.total_resultados), ("Costo total Eccomapp", _decimal_o_vacio(diag.utilidad.costo_productos_universo_utilidad + diag.utilidad.costo_eccomapp_fuera_universo_calculable)), ("Costo utilizado en utilidad", _decimal_o_vacio(diag.utilidad.costo_productos_universo_utilidad)), ("Costo excluido", _decimal_o_vacio(diag.utilidad.costo_eccomapp_fuera_universo_calculable)), ("Grupos calculables", diag.utilidad.grupos_calculables), ("Grupos excluidos", diag.utilidad.grupos_excluidos), ("Fórmula utilidad preliminar", "utilidad_preliminar = neto_ml_universo_calculable - costo_eccomapp_universo_calculable"), ("Motivos de exclusión", "; ".join(f"{k}: {v}" for k, v in diag.utilidad.motivos_exclusion.items() if v)), ("Aclaración", "Control operativo preliminar; no es resultado contable ni fiscal definitivo.")):
        ws.append(list(fila))
    _formatear_tabla(ws, moneda_columnas=set(), wrap_columnas={2}, freeze=False)
    for row in ws.iter_rows(min_row=2):
