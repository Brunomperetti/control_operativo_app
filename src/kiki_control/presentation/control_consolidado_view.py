    ...
    return [
        {
            ...
            "Suma reconstruida desde movimientos": formato_importe(m.suma_reconstruida_movimientos_mp),
            "Agregado financiero original": formato_importe(m.neto_financiero_agregado_original_mp),
            "Diferencia agregado − detalle": formato_importe(m.diferencia_agregado_detalle_mp),
            "Coherencia del grupo": "Sí" if m.coherencia_grupo else "No",
            "Estado de coherencia": m.estado_coherencia.value,
            "Motivo de coherencia": m.motivo_coherencia,
            "Advertencia": m.advertencia_inconsistencia,
